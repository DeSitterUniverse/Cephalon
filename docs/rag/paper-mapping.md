...
| Typed table execution | T-RAG, T2-RAGBench, OpenScholar | A validated application plan performs bounded lookup, filtering, sorting, grouping, aggregation, comparison, difference, and percentage arithmetic over normalized cells. The deterministic language router handles unambiguous lookup/filter/sort/aggregate/count questions; grouping and binary arithmetic remain explicit-plan-only. Ambiguous expressed-unit questions return all cited candidates deterministically. | No model-generated SQL, planner-model call, semantic selection among ambiguous values, cross-document arithmetic, inferred operands, or recursive execution loop. |
